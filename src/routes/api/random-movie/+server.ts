...
export const GET: RequestHandler = async () => {
  ...
  const response = await fetch(`https://api.themoviedb.org/3/discover/movie?api_key=${TMDB_API_KEY}&include_adult=false&with_original_language=en&include_video=false&sort_by=popularity.desc&page=${random(1, 20)}`);
  ...
  while (!movie || movie.budget == 0 || !rating) {
    ...
    const response = await fetch(`https://api.themoviedb.org/3/movie/${popular_movies.results[random(0, 20)].id}?api_key=${TMDB_API_KEY}&append_to_response=external_ids`);
